...
def strip_model_preamble(text):
    ...
    patterns = [
        ...
        r"^\s*以下是.{0,80}(转写|视频内容|整理后|精炼后|Markdown|课程内容).*$",
        r"^\s*这是.{0,80}(转写|视频内容|整理后|精炼后|Markdown|课程笔记).*$",
        r"^\s*已根据.{0,40}(图片|关键帧|专业术语).*$",
        r"^\s*下面是.{0,80}(转写|视频内容|整理后|精炼后|Markdown|课程内容).*$",
        r"^\s*这是一份.{0,80}(精炼后|整理后).{0,80}(Markdown|课程笔记|关键视觉信息).*$",
